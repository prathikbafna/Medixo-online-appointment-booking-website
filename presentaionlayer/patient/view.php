	...
	<table class="table2">
		...
		if(mysqli_num_rows($result3)>=1){
			...
			while ($row3=$result3->fetch_assoc()) {
				...
				echo "<tr><td>".$row3["AppoID"]."</td><td>".$row3["Date"]."</td><td>".$row3["Time"]."</td><td>".$row3["docID"]."</td><td>".$row3['Doctorname']."</td><td>".$row3['Address']."</td><td>".$row3['ContactNumber']."</td> <td>".$row3["categorey"]."</td><td><form method=\"post\" acion= \"cancel.php\" > <input type=\"hidden\" name=\"AppoID2\" value = ".$row3["AppoID"]." ><button type=\"submit\" name=\"cancel\" style=\"box-shadow:none; border:0; border-radius:0px; background:none; outline:none; cursor: pointer;\" type=\"button\"><img style=\"width:25px;height:25px\"src=\"cancel.png\"></button></td></tr>";
			}
...
-->
<script>
	function delete_app(id,uid){
		var xml=new XMLHttpRequest();
		var b;
		console.log("cc"+id+"gg"+uid)
		...
		console.log("xxx1")
		xml.open("POST","delete.php?id="+id +"&userid" +uid ,true);
		xml.send();
